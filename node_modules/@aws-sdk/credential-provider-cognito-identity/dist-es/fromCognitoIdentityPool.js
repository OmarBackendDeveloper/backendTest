...
import { fromCognitoIdentity } from "@aws-sdk/credential-provider-cognito-identity/dist-es/fromCognitoIdentity";
import { localStorage } from "@aws-sdk/credential-provider-cognito-identity/dist-es/localStorage";
import { resolveLogins } from "@aws-sdk/credential-provider-cognito-identity/dist-es/resolveLogins";
export function fromCognitoIdentityPool({ accountId, cache = localStorage(), client, clientConfig, customRoleArn, identityPoolId, logins, userIdentifier = !logins || Object.keys(logins).length === 0 ? "ANONYMOUS" : undefined, logger, parentClientConfig, }) {
    ...
    const cacheKey = userIdentifier
        ...
        : undefined;
    let provider = async () => {
        const { GetIdCommand, CognitoIdentityClient } = await import("@aws-sdk/credential-provider-cognito-identity/dist-es/loadCognitoIdentity");
